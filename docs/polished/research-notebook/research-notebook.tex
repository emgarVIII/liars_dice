...
\section*{Main Page - Goals}\label{main-page---goals}
\addcontentsline{toc}{section}{Main Page - Goals}

\section*{\texorpdfstring{\textbf{\ul{CS370}}}{CS370}}\label{cs370}
\addcontentsline{toc}{section}{\textbf{\ul{CS370}}}

\textbf{\ul{Multiagent Learning \& Computational Game Solving, Notes \&
Planning}}

\subsection*{\texorpdfstring{\textbf{\ul{Deliverables:}}}{Deliverables:}}\label{deliverables}
\addcontentsline{toc}{subsection}{\textbf{\ul{Deliverables:}}}
...
\begin{itemize}
\item
  Paper Selection: Choose a central applied paper within the fields of
  multiagent learning or computational game solving.
\end{itemize}

\subsection{Summary Contents:}\label{summary-contents}

\begin{itemize}
\item
  Key Findings: Summarize the main results and conclusions of the paper.
\item
  Techniques Used: Describe the methodologies and approaches employed in
  the research.
\item
  Future Work Directions: Identify and discuss potential avenues for
...
\begin{itemize}
\item
  Paper Selection: Choose a central theoretical paper within the fields
  of multiagent learning or computational game solving.
\end{itemize}

\subsection{Summary Contents:}\label{summary-contents-1}
...
\begin{itemize}
\item
  \textbf{Due Date: end of semester}
\item
  \textbf{Length:} Minimum 2000 words
\end{itemize}

\subsection{\texorpdfstring{\textbf{Requirements:}}{Requirements:}}\label{requirements-2}

\subsection{Research Direction:}\label{research-direction}

\begin{itemize}
\item
  Novelty and Tractability: Identify a research direction that is both
  novel and feasible to pursue.
\item
  Based on Readings: the proposal should be grounded in insights gained
  from your previous readings and assignments
\end{itemize}

\subsection{Proposal Contents:}\label{proposal-contents}

\begin{itemize}
\item
  Planning note: use this section to organize the final proposal
  direction.
\end{itemize}

\subsection*{\texorpdfstring{\textbf{\ul{Assignment
1:}}}{Assignment 1:}}\label{assignment-1}
\addcontentsline{toc}{subsection}{\textbf{\ul{Assignment 1:}}}

\section*{Sandholm\textquotesingle s Computational Game
Solving}\label{sandholms-computational-game-solving}
\addcontentsline{toc}{section}{Sandholm\textquotesingle s Computational
Game Solving}

\section*{\texorpdfstring{\textbf{\ul{Sandholm\textquotesingle s
Computational Game
Solving}}}{Sandholm\textquotesingle s Computational Game Solving}}\label{sandholms-computational-game-solving-1}
\addcontentsline{toc}{section}{\textbf{\ul{Sandholm\textquotesingle s
Computational Game Solving}}}

\subsection*{\texorpdfstring{\textbf{\ul{IMPORTANT
TERMINOLOGY:}}}{IMPORTANT TERMINOLOGY:}}\label{important-terminology}
\addcontentsline{toc}{subsection}{\textbf{\ul{IMPORTANT TERMINOLOGY:}}}
...
\subsection*{\texorpdfstring{\textbf{\ul{NOTES:}}}{NOTES:}}\label{notes}
\addcontentsline{toc}{subsection}{\textbf{\ul{NOTES:}}}
...
\subsection{Examples of incomplete-information games with sequential \&
simultaneous
moves:}\label{examples-of-incomplete-information-games-with-sequential-simultaneous-moves}
...
\item
  A robot facing adversaries in uncertain, stochastic environment
\item
...
\subsection{Perfect-information games:}\label{perfect-information-games}
...
Texas Hold\textquotesingle em Rules Reference
...
\section*{Sandholm's Research Paper}\label{sandholms-research-paper}
\addcontentsline{toc}{section}{Sandholm's Research Paper}
...
\subsection{Even if Nash is found:}\label{even-if-nash-is-found}

\begin{itemize}
\item
  Moreover, even if a Nash equilibrium could be computed efficiently in
  a game with more than two players, it is not clear that playing such
  an equilibrium strategy would be wise. If each player in such a game
  independently computes and plays a Nash equilibrium, the list of
  strategies that they play (one strategy per player) may not be a Nash
  equilibrium and players might have an incentive to deviate to a
  different strategy.
\end{itemize}

\includegraphics[width=7.5in,height=8.80556in]{media/media/image4.png}

\subsection*{\texorpdfstring{\textbf{\ul{Pluribus:}}}{Pluribus:}}\label{pluribus}
\addcontentsline{toc}{subsection}{\textbf{\ul{Pluribus:}}}
...
\subsection{HOW WAS THE STRATEGY MADE:}\label{how-was-the-strategy-made}

\begin{itemize}
\item
  The core of Pluribus's strategy was computed through self-play, in
  which the AI plays against copies of itself, without any data of human
  or prior AI play used as input. The AI starts from scratch by playing
  randomly and gradually improves as it determines which actions, and
  which probability distribution over those actions, lead to better
  outcomes against earlier versions of its strategy. Forms of self-play
  have previously been used to generate powerful AIs in two-player
  zero-sum games such as backgammon (18), Go (9, 19), Dota 2 (20),
  StarCraft 2 (21), and two-player poker (4--6), though the precise
  algorithms that were used have varied widely. Although it is easy to
  construct toy games with more than two players in which commonly used
  self-play algorithms fail to converge to a meaningful solution (22),
  \textbf{\ul{in practice self-play has nevertheless been shown to do
  reasonably well in some games with more than two players}}
\end{itemize}

\subsection{\texorpdfstring{\textbf{\ul{HOW IT
WORKS:}}}{HOW IT WORKS:}}\label{how-it-works}

\begin{itemize}
\item
  \textbf{\ul{(PHASE 1)}} Pluribus's self-play produces a strategy for
  the entire game offline, which we refer to as the blueprint strategy.
...
\subsection{\texorpdfstring{\textbf{\ul{Prices \& hardware
allocations:}}}{Prices \& hardware allocations:}}\label{prices-hardware-allocations}

\begin{itemize}
\item
  The blueprint strategy for Pluribus was computed in 8 days on a
  64-core server for a total of 12,400 CPU core hours. It required less
  than 512 GB of memory. At current cloud computing spot instance rates,
  this would cost about \$144 to produce.
\item
...
\subsection{\texorpdfstring{\textbf{\ul{Action
abstraction:}}}{Action abstraction:}}\label{action-abstraction}

\begin{itemize}
\item
  reduces the number of different actions the AI needs to consider.
\item
...
\item
  ...
  depending on the situation. Although Pluribus can limit itself to only
  betting one of a few different sizes between \$100 and \$10,000, when
  actually playing no-limit poker, \textbf{the opponents are not
  constrained to those few options.}
\item
  What happens if an opponent bets \$150 while Pluribus has only been
  trained to consider bets of \$100 or \$200? Generally, Pluribus will
  rely on its search algorithm (described in a later section) to compute
  a response in real-time to such ``off-tree'' actions.
\end{itemize}

\subsection{\texorpdfstring{\textbf{\ul{Information
abstraction:}}}{Information abstraction:}}\label{information-abstraction}

\begin{itemize}
\item
  decision points that are similar in terms of what information has been
  revealed (in poker, the player's cards and revealed board cards) are
  bucketed together and treated identically
\item
  For example, a 10-high straight and a 9-high straight are distinct
  hands but are nevertheless strategically similar. Pluribus may bucket
  these hands together and treat them identically, thereby reducing the
  number of distinct situations for which it needs to determine a
  strategy.
\item
  drastically reduces the complexity of the game, but it may wash away
  subtle differences that are important for superhuman performance.
\item
  Therefore, during actual play against humans, Pluribus uses
  information abstraction only to reason about situations on future
  betting rounds, never the betting round that it is actually in.
  Information abstraction is also applied during offline self-play
\end{itemize}

\subsection{\texorpdfstring{\href{https://aws.amazon.com/what-is/monte-carlo-simulation/}{\ul{Montecarlo
simulation:}}}{Montecarlo simulation:}}\label{montecarlo-simulation}

\begin{itemize}
\item
  a computational algorithm that uses random sampling to solve problems.
  It\textquotesingle s used to approximate functions and simulate the
...
\item
  ...
  \begin{itemize}
  \item
    Computer programs use this method to analyze past data and predict a
    range of future outcomes based on a choice of action.
  \item
...
\item
  ...
  different results each time. \textbf{For example, the distance between
  your home and office is fixed. However, a probabilistic simulation
  might predict different travel times by considering factors such as
  congestion, bad weather, and vehicle breakdowns.}
\item
  In contrast, conventional forecasting methods are more deterministic.
  They provide a definite answer to the prediction and cannot factor in
  uncertainty. For instance, they might tell you the minimum and maximum
  travel time, but both answers are less accurate.
\end{itemize}

\includegraphics[width=7.5in,height=6.91667in]{media/media/image2.png}

\subsection*{\texorpdfstring{\textbf{Translating\,Pluribus‑style
multi‑agent game‑solving to a Mario\,Kart racing
AI:}}{Translating\,Pluribus‑style multi‑agent game‑solving to a Mario\,Kart racing AI:}}\label{translating-pluribusstyle-multiagent-gamesolving-to-a-mario-kart-racing-ai}
\addcontentsline{toc}{subsection}{\textbf{Translating\,Pluribus‑style
multi‑agent game‑solving to a Mario\,Kart racing AI:}}
...
\section*{Mario Kart Research
Direction}\label{mario-kart-research-direction}
\addcontentsline{toc}{section}{Mario Kart Research Direction}

\section*{Pluribus to Pokemon
Battles}\label{pluribus-to-pokemon-battles}
\addcontentsline{toc}{section}{Pluribus to Pokemon Battles}
...
\item
  ...
  Pokemon, moves are sequential (both players make a choice, and that's
  it)
\item
...
\section*{CFR Web Simulation Notes}\label{cfr-web-simulation-notes}
\addcontentsline{toc}{section}{CFR Web Simulation Notes}
